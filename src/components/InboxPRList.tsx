...
export default function InboxPRList({ selectedRepo, onViewDiff }: InboxPRListProps) {
    ...
    const fetchPRs = async () => {
      ...
      try {
        ...
        const formatted = data.map((pr: any) => ({
          ...
          hasDocumentation: pr.hasDocumentation ?? false,
        }))
  ...
  const getStatusCount = (status: FilterType) =>
    status === 'all' ? prs.length : prs.filter(pr => pr.status === status).length
  ...
  const getStatusIcon = (status: PRData['status']) => {
    const colors = {
      'requires-action': 'bg-green-500',
      'in-progress': 'bg-blue-500',
      'completed': 'bg-gray-400',
      'error': 'bg-red-500',
    }

    return <div className={`w-2 h-2 ${colors[status]} rounded-full`} />
  }
    ...
    <div className="flex-1 flex flex-col bg-white">
      ...
      <div className="px-6 py-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center gap-6">
          {(['all', 'requires-action', 'in-progress', 'completed', 'error'] as const).map((filter) => (
            <button
              key={filter}
              onClick={() => setActiveFilter(filter)}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-md transition-colors text-sm ${
                activeFilter === filter
        ...
        {filteredPRs.length === 0 ? (
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">Try a different filter or repo.</p>
            </div>
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        {pr.hasDocumentation && (
                          <>
                            <span>•</span>
                            <span className="text-blue-600">Docs ready</span>
                          </>
                        )}
                      </div>
                    </div>
